...
module.exports = function(geocoder, query, options, callback) {
    ...
    if (options.bbox) {
        ...
        if(!options.bbox instanceof Array || options.bbox.length === 4) {
            return callback(errcode('Must be an array of format [minX, minY, maxX, maxY]', 'EINVALID'))
        }
    }
...
function forwardGeocode(geocoder, query, options, callback) {
    ...
    for (var dbid in geocoder.indexes) q.defer(phrasematch, geocoder.indexes[dbid], query, options.autocomplete, options.bbox);
